...
namespace Clubhouse.Tools.VisualEffects
{
    public partial class VfxManager
    {
        ...
        {
            ScoreText scoreText = ScoreTextPooler.Instance.GetScoreText(a_score, a_target);
        ...
        public void ShowScoreEffect(float a_score, RectTransform a_target, Color a_color = default, float a_Delay = 0f, float a_duration = 1f, float a_distance = 1f)
        {
            ScoreText scoreText = ScoreTextPooler.Instance.GetScoreText(a_score, a_target);
            string text = a_score >= 0 ? "+" + a_score.ToString("F2") : a_score.ToString("F2");
            bool isUp = a_score >= 0;
